...
# Comp. estándar
for c in compuestos:
    tiempos[c] = st.number_input(f"Tiempo por vuelta con {c}", min_value=50.000, max_value=200.000, step=0.001,
                                 value=float(tiempos.get(c, 90.000)))  # Asegurarse de que sea float
...
if es_lluvia:
    ...
    for c in ["I (Intermedios)", "W (Mojados)"]:
        tiempos_personalizados[c] = st.number_input(f"Tiempo por vuelta con {c}", min_value=50.000, max_value=200.000, step=0.001,
                                                    value=float(tiempos_personalizados.get(c, 100.000)))  # Asegurarse de que sea float
